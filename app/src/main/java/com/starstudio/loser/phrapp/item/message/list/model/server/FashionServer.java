...
public class FashionServer extends BaseApiServer {
    private static FashionServer sInstance;
    private Api mApi;
    private final String key = "b0cd78efd4f4516ba45b4a747ae6d956";


    private FashionServer(Api api) {
        this.mApi = api;
    }

    public static synchronized FashionServer getInstance(Context context) {
        if (sInstance == null) {
            sInstance = new FashionServer(Client.getInstance(context).getClient().create(Api.class));
        }
        return sInstance;
    }

    public Observable<BaseBean> fetchInfo() {
        return mApi.fetchData("shishang", key);
    }
}
